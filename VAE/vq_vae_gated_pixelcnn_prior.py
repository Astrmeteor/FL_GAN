...
def train(args, data_shape, model, optimizer, no_epochs, train_loader, validation_loader, test_loader, privacy_engine, prior_only=False):
    """
    ...
    for epoch in tqdm.tqdm(range(no_epochs)):
        ...
        print(f"\n[INFO] Validation ...")
        validate_loss = get_batched_loss(args, validation_loader, model, loss_fct, loss_triples=False)
        validate_losses.append(validate_loss)
        print(f"\n[INFO] Test ...")
        test_loss = get_batched_loss(args, test_loader, model, loss_fct, loss_triples=False)
        ...
        if not prior_only:
            ...
            def sample(no_samples=args.num_sampling):
                ...
                encoding_one_hot = torch.zeros(encoding_inds.size(0), args.num_embeddings, device=device)
                encoding_one_hot.scatter_(1, encoding_inds, 1)  # [BHW x K]
                quantized_latents = torch.matmul(encoding_one_hot, model.codebook.codebook.weight)  # [BHW, D]
                quantized_latents = quantized_latents.view(latents_shape + (args.embedding_dim,))  # [B x H x W x D]
